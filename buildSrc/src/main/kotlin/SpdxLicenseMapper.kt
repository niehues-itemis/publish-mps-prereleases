 ...
 * - Map common license names to their official SPDX identifiers
 * - Handle SPDX license expressions with exceptions (using WITH operator)
 * - Extract and validate licenses from JSON files
 */
object SpdxLicenseMapper {
    
    /**
     * Converts a license name to its SPDX identifier.
     * Handles both simple licenses and license expressions with exceptions (e.g., "GPL 2.0 + Classpath").
     * 
     * @param licenseName The license name to convert
     * @return The SPDX identifier, or null if no mapping exists
     */
    fun toSpdxIdentifier(licenseName: String): String? {
        val trimmed = licenseName.trim()
        
        // Handle SPDX expressions with "+" (e.g., "GPL 2.0 + Classpath" -> "GPL-2.0-only WITH Classpath-exception-2.0")
        if (trimmed.contains("+")) {
            ...
            if (parts.size == 2) {
                ...
                if (mainLicense == null || exception == null) {
                    return null
                }
    ...
    /**
     * Maps exception names to their SPDX exception identifiers.
     * 
     * @param exceptionName The exception name to map
     * @return The SPDX exception identifier, or null if no mapping exists
     */
    ...
    /**
     * Maps simple license names to their SPDX identifiers.
     * 
     * @param licenseName The license name to map
     * @return The SPDX identifier, or null if no mapping exists
     */
    fun toSpdxIdentifierSimple(licenseName: String): String? {
        return when (licenseName.trim()) {
            ...
            // Unofficial licenses using LicenseRef
